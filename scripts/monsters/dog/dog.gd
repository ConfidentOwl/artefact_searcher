...
var dog_curent_state : dog_state = dog_state.idle
@onready var animation_player: AnimationPlayer = $AnimationPlayer

@onready var skeleton_3d: Skeleton3D = $dog_ogf/Skeleton3D
	...
	match dog_curent_state :
		dog_state.idle :
			...
			match randi_range(0, 1):
				...
				1: 
					animation_player.play("stand_idle_1")
					_previev_idle_animation = "stand_idle_1"
				_:
					printerr("Get int niot indefine in animtion selector - dog_state.idle ")
				#animation_player.play(_previev_idle_animation,-1,-1,true)
				#_previev_idle_animation = ""
		dog_state.walk:
			random_walk_animtion()
			var curent_pos : Vector3 = global_transform.origin
			var next_location :Vector3= navigation_agent_3d.get_next_path_position()
		...
		dog_state.beat:
			...
			if _damage_cool_down <=0 :
				_damage_cool_down = animation_player.get_animation(animation_player.get_current_animation()).length
				var target :Node3D= targets[0]
				if target is Player :
					target.get_damage(ATACK_DAMAGE)
					target.get_blooding(ATACK_DAMAGE/10 )
					if 0.25 < randf():
						var can : bool = get_random_move_new_point(15)
						_run_around = true 
		...
		dog_state.dead:
			create_loot()
			animation_player.play("lie_sleep_0",0,1,false)
		#_: 
...
func get_random_point_around(_rand_offset : int ) -> Vector3:
	var rand_offset : int = _rand_offset
...
func get_random_move_new_point(_rand_offset : int ) -> bool:
	var new_rand_point : Vector3 = get_random_point_around(_rand_offset)
...
func random_walk_animtion() -> void:
	if animation_player.current_animation.contains("stand_walk_fwd_0") :
		return
...
func random_run_animation() -> void:
	...
	match randi_range(0, 1):
		0:
			animation_player.play("stand_run_0",0,1,false)
		1:
			animation_player.play("stand_run_attack_0",0,1,false)
...
func _on_looking_body_entered(body: Node3D) -> void:
	if body.is_in_group("stalker"):
		targets.append(body)
		coling_help.call_around(body.position)


func _on_looking_body_exited(body: Node3D) -> void:
	targets.erase(body)
